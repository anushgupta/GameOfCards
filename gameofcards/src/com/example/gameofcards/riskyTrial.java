...
public class riskyTrial extends Activity implements View.OnClickListener, RoomRequestListener, NotifyListener, TurnBasedRoomListener, UpdateRequestListener {
	...
	public void onClick(View v) {
		...
		case R.id.bPlayer1:
			try{
				...
				Toast t = Toast.makeText(riskyTrial.this,
	...
	public void doCardAllocation(){
		...
		for(int i=0;i<4;i++){
			...
			playerCards[(player.getId()-1)*4+i]=x;
			myHandCards[i]=x;
			myCards[x]=1;
			Toast.makeText(mContext, Integer.toString(playerCards[i]), Toast.LENGTH_SHORT).show();	
		}
	...
	public void addNewPlayer(final String name,final int flag){
		...
		runOnUiThread(new Runnable() {
            ...
            public void run() {
            	...
            	if(numberOfPlayers==4){
            		...
            		//if(joined[3]==1 && set[3]==1){	
            			if(player.getId()==1){
            				doCardAllocation();
            				String cardy="C2";
            				for(int i=0;i<4;i++){
            					cardy+= " " + Integer.toString(playerCards[i]);
            				}
            				Toast.makeText(mContext, cardy, Toast.LENGTH_SHORT).show();
            				
            				myGame.sendUpdatePeers(cardy.getBytes());
            			}
            			if(player.getId()==2){
            				doCardAllocation();
            				String cardy="C3";
		...
		runOnUiThread(new Runnable() {
            ...
            public void run() {
            	...
            	if(name.length==4){
            		...
            		set[0]=1;
            		set[1]=1;
            		addNewPlayer(name[1],1);
            	}
            	else if(name.length==1){
            		p1.setText(name[0]);
            		player.setId(1);
            		set[0]=1;
            		addNewPlayer(name[0],1);
            	}
            }
		 });
	}
	...
	public void onUpdatePeersReceived(UpdateEvent event) {
		...
		if(movedata.charAt(0)=='C'){
			runOnUiThread(new Runnable() {
	            @Override
	            public void run() {
	            	
	            	String suby=movedata.substring(3);
	            	int jIndex=0;
	            	int idCard=(int)movedata.charAt(1)-(int)'0';
	            	if(player.getId()==2){	
	            		jIndex=4;
	            		Toast.makeText(mContext,"In suby with playerid:2", Toast.LENGTH_SHORT).show();
    			
	            		StringTokenizer strToken=new StringTokenizer(suby);
	            		for(int i=0;i<jIndex;i++){
	            			playerCards[i]=Integer.parseInt((String)strToken.nextElement());
	            			myCards[playerCards[i]]=1;
	            		}
	            	}
	            	if(player.getId()==3){	
	            		jIndex=8;
